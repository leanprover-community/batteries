...
/--
info: theorem ex₁ : ∀ (x y z : Nat), max (0 + max x (max z (max (0 + 0) (max 1 0 + 0 + 0) * y))) y = max (max x y) z :=
fun x y z =>
  Eq.mpr
    (id
      (congr
        (congrArg Eq
          (Eq.trans
            (congrArg (fun x => max x y)
              (Eq.trans
                (congrArg (HAdd.hAdd 0)
                  (Eq.trans
                    (congrArg (fun x_1 => max x (max z x_1))
                      (Eq.trans
                        (congrArg (fun x => x * y)
                          (Eq.trans
                            (congr
                              (congrArg max
                                (Data.AC.Context.eq_of_norm
                                  { op := HAdd.hAdd, assoc := instAssociativeNatHAdd,
                                    comm := some { down := instCommutativeNatHAdd }, idem := none,
                                    vars :=
                                      [{ value := 0, neutral := some { down := LawfulCommIdentity.toLawfulIdentity } }],
                                    arbitrary := 0 }
                                  (Data.AC.Expr.op (Data.AC.Expr.var 0) (Data.AC.Expr.var 0)) (Data.AC.Expr.var 0)
                                  (Eq.refl true)))
                              (Eq.trans
                                (congrArg (fun x => x + 0 + 0)
                                  (Data.AC.Context.eq_of_norm
                                    { op := max, assoc := instAssociativeNatMaxInstMaxNat,
                                      comm := some { down := instCommutativeNatMaxInstMaxNat },
                                      idem := some { down := instIdempotentOpNatMaxInstMaxNat },
                                      vars :=
                                        [{ value := 0,
                                            neutral := some { down := LawfulCommIdentity.toLawfulIdentity } },
                                          { value := 1, neutral := none }],
                                      arbitrary := 0 }
                                    (Data.AC.Expr.op (Data.AC.Expr.var 1) (Data.AC.Expr.var 0)) (Data.AC.Expr.var 1)
                                    (Eq.refl true)))
                                (Data.AC.Context.eq_of_norm
                                  { op := HAdd.hAdd, assoc := instAssociativeNatHAdd,
                                    comm := some { down := instCommutativeNatHAdd }, idem := none,
                                    vars :=
                                      [{ value := 0, neutral := some { down := LawfulCommIdentity.toLawfulIdentity } },
                                        { value := 1, neutral := none }],
                                    arbitrary := 0 }
                                  (Data.AC.Expr.op (Data.AC.Expr.op (Data.AC.Expr.var 1) (Data.AC.Expr.var 0))
                                    (Data.AC.Expr.var 0))
                                  (Data.AC.Expr.var 1) (Eq.refl true))))
                            (Data.AC.Context.eq_of_norm
                              { op := max, assoc := instAssociativeNatMaxInstMaxNat,
                                comm := some { down := instCommutativeNatMaxInstMaxNat },
                                idem := some { down := instIdempotentOpNatMaxInstMaxNat },
                                vars :=
                                  [{ value := 0, neutral := some { down := LawfulCommIdentity.toLawfulIdentity } },
                                    { value := 1, neutral := none }],
                                arbitrary := 0 }
                              (Data.AC.Expr.op (Data.AC.Expr.var 0) (Data.AC.Expr.var 1)) (Data.AC.Expr.var 1)
                              (Eq.refl true))))
                        (Data.AC.Context.eq_of_norm
                          { op := HMul.hMul, assoc := instAssociativeNatHMul,
                            comm := some { down := instCommutativeNatHMul }, idem := none,
                            vars :=
                              [{ value := y, neutral := none },
                                { value := 1, neutral := some { down := LawfulCommIdentity.toLawfulIdentity } }],
                            arbitrary := y }
                          (Data.AC.Expr.op (Data.AC.Expr.var 1) (Data.AC.Expr.var 0)) (Data.AC.Expr.var 0)
                          (Eq.refl true))))
                    (Data.AC.Context.eq_of_norm
                      { op := max, assoc := instAssociativeNatMaxInstMaxNat,
                        comm := some { down := instCommutativeNatMaxInstMaxNat },
                        ...
                        arbitrary := x }
                      (Data.AC.Expr.op (Data.AC.Expr.var 0) (Data.AC.Expr.op (Data.AC.Expr.var 2) (Data.AC.Expr.var 1)))
                      (Data.AC.Expr.op (Data.AC.Expr.var 0) (Data.AC.Expr.op (Data.AC.Expr.var 1) (Data.AC.Expr.var 2)))
                      (Eq.refl true))))
                (Data.AC.Context.eq_of_norm
                  { op := HAdd.hAdd, assoc := instAssociativeNatHAdd, comm := some { down := instCommutativeNatHAdd },
                    idem := none,
                    vars :=
                      [{ value := 0, neutral := some { down := LawfulCommIdentity.toLawfulIdentity } },
                        { value := max x (max y z), neutral := none }],
                    arbitrary := 0 }
                  (Data.AC.Expr.op (Data.AC.Expr.var 0) (Data.AC.Expr.var 1)) (Data.AC.Expr.var 1) (Eq.refl true))))
            (Data.AC.Context.eq_of_norm
              { op := max, assoc := instAssociativeNatMaxInstMaxNat,
                comm := some { down := instCommutativeNatMaxInstMaxNat },
                idem := some { down := instIdempotentOpNatMaxInstMaxNat },
                vars :=
                  [{ value := x, neutral := none }, { value := y, neutral := none }, { value := z, neutral := none }],
                arbitrary := x }
              (Data.AC.Expr.op
                (Data.AC.Expr.op (Data.AC.Expr.var 0) (Data.AC.Expr.op (Data.AC.Expr.var 1) (Data.AC.Expr.var 2)))
                (Data.AC.Expr.var 1))
        ...
        (Data.AC.Context.eq_of_norm
          { op := max, assoc := instAssociativeNatMaxInstMaxNat,
            comm := some { down := instCommutativeNatMaxInstMaxNat },
            idem := some { down := instIdempotentOpNatMaxInstMaxNat },
            vars := [{ value := x, neutral := none }, { value := y, neutral := none }, { value := z, neutral := none }],
            arbitrary := x }
          (Data.AC.Expr.op (Data.AC.Expr.op (Data.AC.Expr.var 0) (Data.AC.Expr.var 1)) (Data.AC.Expr.var 2))
          (Data.AC.Expr.op (Data.AC.Expr.var 0) (Data.AC.Expr.op (Data.AC.Expr.var 1) (Data.AC.Expr.var 2)))
          (Eq.refl true))))
...
/--
info: theorem ex₃ : ∀ (n : Nat), (fun x => n + x) = fun x => x + n :=
fun n =>
  Eq.mpr
    (id
      (congr
        (congrArg Eq
          (funext fun x =>
            Data.AC.Context.eq_of_norm
              { op := HAdd.hAdd, assoc := instAssociativeNatHAdd, comm := some { down := instCommutativeNatHAdd },
                idem := none, vars := [{ value := n, neutral := none }, { value := x, neutral := none }],
                arbitrary := n }
              (Data.AC.Expr.op (Data.AC.Expr.var 0) (Data.AC.Expr.var 1))
              (Data.AC.Expr.op (Data.AC.Expr.var 0) (Data.AC.Expr.var 1)) (Eq.refl true)))
        (funext fun x =>
          Data.AC.Context.eq_of_norm
            { op := HAdd.hAdd, assoc := instAssociativeNatHAdd, comm := some { down := instCommutativeNatHAdd },
              idem := none, vars := [{ value := n, neutral := none }, { value := x, neutral := none }], arbitrary := n }
            (Data.AC.Expr.op (Data.AC.Expr.var 1) (Data.AC.Expr.var 0))
            (Data.AC.Expr.op (Data.AC.Expr.var 0) (Data.AC.Expr.var 1)) (Eq.refl true))))
    (Eq.refl fun x => n + x)
-/
#guard_msgs in
