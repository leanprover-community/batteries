...
-- Note: there are further tests in Mathlib, in `test/Simps.lean`.
...
example (n : Nat) : foo.rfl.toFun n = n := by rw [foo.rfl_toFun, id]
example (n : Nat) : foo.rfl.invFun n = n := by rw [foo.rfl_invFun]
...
example {a : Nat} {h : 1 = a} : foo.1 = a := by rw [foo_fst, h]
example {a : Nat} {h : 1 = a} : foo.1 = a := by simp; rw [h]
example {a : Int} {h : 2 = a} : foo.2 = a := by simp; rw [h]
example {a : Nat} {h : 1 = a} : foo.1 = a := by dsimp; rw [h] -- check that dsimp also unfolds
example {a : Int} {h : 2 = a} : foo.2 = a := by dsimp; rw [h]
...
/- check some failures -/
def bar1 : Nat := 1 -- type is not a structure
...
def my_equiv := Equiv'
@[simps] def baz : my_equiv Nat Nat := ⟨id, λ x => x, λ _ => rfl, λ _ => rfl⟩
...
@[simps]
def nested1 : MyProd Nat $ MyProd Int Nat :=
  ⟨2, -1, 1⟩

@[simps (config := .lemmasOnly)]
def nested2 : Nat × MyProd Nat Nat :=
...
@[simps] def bar {_ : Type _} (n m : Nat) : Nat × Int :=
⟨n - m, n + m⟩
...
structure VeryPartiallyAppliedStr :=
  (data : ∀β, Nat → β → MyProd Nat β)

/- if we have a partially applied constructor, we treat it as if it were eta-expanded.
  (this is not very useful, and we could remove this behavior if convenient) -/
@[simps]
def very_partially_applied_term : VeryPartiallyAppliedStr := ⟨@MyProd.mk Nat⟩
...
@[simps] def let1 : Nat × Int :=
let n := 3; ⟨n + 4, 5⟩

@[simps] def let2 : Nat × Int :=
let n := 3; let m := 4; let k := 5; ⟨n + m, k⟩

@[simps] def let3 : Nat → Nat × Int :=
λ n => let m := 4; let k := 5; ⟨n + m, k⟩

@[simps] def let4 : Nat → Nat × Int :=
...
@[simps fst] def specify1 : Nat × Nat × Nat := (1, 2, 3)
@[simps snd] def specify2 : Nat × Nat × Nat := (1, 2, 3)
@[simps snd_fst] def specify3 : Nat × Nat × Nat := (1, 2, 3)
@[simps snd snd_snd] def specify4 : Nat × Nat × Nat := (1, 2, 3) -- last argument is ignored
@[simps] noncomputable def specify5 : Nat × Nat × Nat := (1, Classical.choice ⟨(2, 3)⟩)
end specify
...
@[simps (config := {simpRhs := true})] def myNatEquiv : Nat ≃ Nat :=
  ⟨λ n => 0 + n, λ n => 1 * n * 1, by intro n; simp, by intro n; simp⟩

example (n : Nat) : myNatEquiv.toFun (myNatEquiv.toFun $ myNatEquiv.invFun n) = n := by
  { /-successIfFail { rfl },-/ simp only [myNatEquiv_toFun, myNatEquiv_invFun] }

@[simps (config := {simpRhs := true})] def succeed_without_simplification_possible : Nat ≃ Nat :=
  ⟨λ n => n, λ n => n, by intro n; rfl, by intro n; rfl⟩


/- test that we don't recursively take projections of `prod` and `PProd` -/
@[simps] def pprodEquivProd2 : PProd Nat Nat ≃ Nat × Nat :=
  { toFun := λ x => ⟨x.1, x.2⟩
    invFun := λ x => ⟨x.1, x.2⟩
    left_inv := λ ⟨_, _⟩ => rfl
    right_inv := λ ⟨_, _⟩ => rfl }
...
@[simps! (config := {notRecursive := []})] def pprodEquivProd22 : PProd Nat Nat ≃ Nat × Nat :=
  pprodEquivProd2
...
@[simps] def foo : FooStr := ⟨Nat, 3⟩
@[simps] def foo2 : FooStr := ⟨Nat, 34⟩

example {x : Type} (h : Nat = x) : foo = x := by simp only [foo_c]; rw [h]
example {x : Nat} (h : (3 : Nat) = x) : foo.x = x := by simp only [foo_x]; rw [h]

structure VooStr (n : Nat) :=
 (c : Type)
 (x : c)

instance (n : Nat) : CoeSort (VooStr n) Type := ⟨VooStr.c⟩

@[simps] def voo : VooStr 7 := ⟨Nat, 3⟩
@[simps] def voo2 : VooStr 4 := ⟨Nat, 34⟩

example {x : Type} (h : Nat = x) : voo = x := by simp only [voo_c]; rw [h]
example {x : Nat} (h : (3 : Nat) = x) : voo.x = x := by simp only [voo_x]; rw [h]
...
theorem Prod.mk.inj_iff {a₁ a₂ : α} {b₁ b₂ : β} : (a₁, b₁) = (a₂, b₂) ↔ a₁ = a₂ ∧ b₁ = b₂ :=
  Iff.of_eq (Prod.mk.injEq _ _ _ _)

theorem Prod.ext_iff {p q : α × β} : p = q ↔ p.1 = q.1 ∧ p.2 = q.2 := by
  rw [mk.inj_iff]

@[ext]
theorem Prod.ext {α β} {p q : α × β} (h₁ : p.1 = q.1) (h₂ : p.2 = q.2) : p = q :=
  Prod.ext_iff.2 ⟨h₁, h₂⟩
...
class PropClass (n : Nat) : Prop :=
  (has_true : True)

instance has_PropClass (n : Nat) : PropClass n := ⟨trivial⟩

structure NeedsPropClass (n : Nat) [PropClass n] :=
  (t : True)
...
structure DecoratedEquiv (α : Sort _) (β : Sort _) extends Equiv' α β :=
  (P_toFun  : ∀ ⦃a₁ a₂⦄, toFun a₁ = toFun a₂ → a₁ = a₂)
  (P_invFun : ∀ ⦃a₁ a₂⦄, invFun a₁ = invFun a₂ → a₁ = a₂)
...
structure FurtherDecoratedEquiv (α : Sort _) (β : Sort _) extends DecoratedEquiv α β :=
  (Q_toFun  : ∀ b, ∃ a, toFun a = b)
  (Q_invFun : ∀ b, ∃ a, invFun a = b)
...
@[simps]
def thing (h : Bool ≃ (Bool ≃ Bool)) : AddHomPlus2 (λ _ : Nat => Bool) :=
  { myMul :=
    { toFun := λ b =>
      { toFun := h b
        invFun := (h b).symm
...
example (h : Bool ≃ (Bool ≃ Bool)) (i j : Nat) (b1 b2 : Bool) {x} (h2 : h b1 b2 = x) :
