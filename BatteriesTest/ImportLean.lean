...
If this test fails in your PR because `foo` or `rewriteSyntax` no longer produces an error, you have
almost certainly introduced an ordinary import that exposes Lean's metaprogramming API at runtime
through `Batteries`. Check your new or changed imports and their transitive dependencies, and change
imports used only for elaboration to `meta import`. Tactic implementations should be marked `meta`
and meta-import their Lean dependencies.
-/
