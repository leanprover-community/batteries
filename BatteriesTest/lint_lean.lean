...
-- attribute [nolint simpNF] List.nil_append_fun
...
-- These are all intentional and have `nolint` attributes above.
-- #lint only dupNamespace in all -- Found 6 errors

-- All of these should be caused by `abbrev`.
...
-- -- attribute [nolint explicitVarsOfIff] Iff.refl
-- -- These next three are awkward to use if we follow the `explicitVarsOfIff` rule.
-- -- attribute [nolint explicitVarsOfIff] Std.DHashMap.Internal.Raw₀.isEmpty_eq_false_iff_exists_contains_eq_true
-- -- attribute [nolint explicitVarsOfIff] Std.DHashMap.Internal.Raw₀.isEmpty_iff_forall_contains
-- -- attribute [nolint explicitVarsOfIff] Std.DHashMap.Internal.Raw₀.mem_keys
-- #lint only explicitVarsOfIff in all -- Found 4 errors.
-- #lint only impossibleInstance  in all -- Found 0 errors
-- #lint only simpVarHead in all -- Found 0 error
-- -- This seems to be a bug in the linter.
-- -- attribute [nolint unusedHavesSuffices] Std.DHashMap.Internal.AssocList.length_eq
-- #lint only unusedHavesSuffices in all  -- Found 1 errors
