...
theorem IterM.toArray_scanWithPostCondition [Monad m] [LawfulMonad m] [Iterator α Id β]
  ...
  rw [← toArray_toList, IterM.toList_scanWithPostCondition, Array.scanlM_eq_scanlM_toList]
  rfl
...
@[simp]
theorem IterM.toArray_scan [Iterator α m β] [Finite α m] [Monad m] [LawfulMonad m]
    {f : γ → β → γ} {init : γ} (it : IterM (α := α) m β) :
    (it.scan f init).toArray = Array.scanl f init <$> it.toArray := by
  simp [← toArray_toList, toList_scan, List.toArray_scanl]
...
theorem Iter.toArray_scan [Iterator α Id β] [Finite α Id]
  ...
  rw [← toArray_toList, Iter.toList_scan, Array.scanl_eq_scanl_toList]
  rfl
end Std
