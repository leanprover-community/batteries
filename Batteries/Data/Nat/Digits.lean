...
theorem ofDigitsBE.loop_nil :
    loop (base := base) n [] = n := rfl

theorem ofDigitsBE.loop_cons {l : List (Fin base)} :
    loop n (d :: l) = loop (n * base + d) l := rfl

theorem ofDigitsBE.loop_eq {l : List (Fin base)} :
...
abbrev ofDigitsLETR (l : List (Fin base)) : Nat := ofDigitsBE l.reverse

@[csimp]
theorem ofDigitsLE_eq_ofDigitsLETR : @ofDigitsLE = @ofDigitsLETR := by
...
theorem toDigitsUpToBE.loop_zero [NeZero base] :
    loop base 0 n l = l := rfl

theorem toDigitsUpToBE.loop_succ [NeZero base] :
    loop base (prec+1) n l = loop base prec (n / base) (Fin.ofNat base n :: l) := rfl

theorem toDigitsUpToBE.loop_eq [NeZero base] :
...
theorem toDigitsUpToBE.ofDigitsBE_loop [NeZero base] :
