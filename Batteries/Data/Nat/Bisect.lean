...
theorem avg_ge_left (h : a ≤ b) : a ≤ avg a b := by
  apply (Nat.le_div_iff_mul_le Nat.zero_lt_two).mpr; simp_arith [*]

theorem avg_ge_right (h : b ≤ a) : b ≤ avg a b := by
  apply (Nat.le_div_iff_mul_le Nat.zero_lt_two).mpr; simp_arith [*]
...
theorem bisect_ge_start {p : Nat → Bool} (h : start < stop) (hstart : p start = true)
    (hstop : p stop = false) : bisect h hstart hstop ≥ start := by
  unfold bisect
  simp only; split
  · split
    · next h' _ =>
      have : avg start stop - start < stop - start := by
        ...
        · exact Nat.le_of_lt h'
        · exact avg_lt_right h
      exact bisect_ge_start ..
    · next h' _ =>
      apply Nat.le_trans
      · exact Nat.le_of_lt h'
      · exact bisect_ge_start ..
  · exact avg_ge_left (Nat.le_of_lt h)
...
theorem bisect_true {p : Nat → Bool} (h : start < stop) (hstart : p start = true)
  ...
  · split
    · have : avg start stop - start < stop - start := by
        apply Nat.sub_lt_sub_right
        · exact Nat.avg_ge_left (Nat.le_of_lt h)
        · exact Nat.avg_lt_right h
      exact bisect_true ..
    · exact bisect_true ..
  · next h' =>
...
theorem bisect_false {p : Nat → Bool} (h : start < stop) (hstart : p start = true)
