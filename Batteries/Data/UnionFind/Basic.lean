/- Commented out entire file on nightly-2024-11-12.
/-
...
theorem parentD_eq {arr : Array UFNode} {i : Nat} {h} : parentD arr i = arr[i].parent := dif_pos _

theorem parentD_eq' {arr : Array UFNode} {i} (h) :
    parentD arr i = arr[i].parent := dif_pos _

theorem rankD_eq {arr : Array UFNode} {i : Fin arr.size} : rankD arr i = arr[i].rank := dif_pos _
...
theorem parent'_lt (self : UnionFind) (i : Nat) (h) :
    self.arr[i].parent < self.size := by
...
theorem rank'_lt (self : UnionFind) (i : Fin self.size) : (self.arr[i]).parent ≠ i →
...
theorem rank'_lt_rankMax (self : UnionFind) (i : Fin self.size) :
    (self.arr[i]).rank < self.rankMax := by
...
theorem parent_root (self : UnionFind) (x : Fin self.size) :
    (self.arr[self.root x]).parent = self.root x := by
...
theorem findAux_s {self : UnionFind} {x : Fin self.size} :
    (findAux self x).s = if self.arr[x].parent = x then self.arr else
