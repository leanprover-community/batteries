...
/--
Implementation of the `cases_type` tactic.

Calls `cases` on all hypotheses with head symbol in `heads`.
Prevents calling `cases` when this would produce multiple subgoals if `allowSplit := false`.
Allow recursively calling cases on newly generated matching hypotheses if `recursive := true`.
-/
