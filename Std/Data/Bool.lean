...
@[simp] theorem and_false_left : (false && x) = false := by bool_tt x
@[simp] theorem and_false_right : (x && false) = false := by bool_tt x
@[simp] theorem and_true_left : (true && x) = x := by bool_tt x
@[simp] theorem and_true_right : (x && true) = x := by bool_tt x
@[simp] theorem and_not_self_left : (!x && x) = false := by bool_tt x
@[simp] theorem and_not_self_right : (x && !x) = false := by bool_tt x
@[simp] theorem and_idem : (x && x) = x := by bool_tt x
...
@[local simp] theorem and_deMorgan : (!(x && y)) = (!x || !y) := by bool_tt x y
...
@[simp] theorem or_false_left : (false || x) = x := by bool_tt x
@[simp] theorem or_false_right : (x || false) = x := by bool_tt x
@[simp] theorem or_true_left : (true || x) = true := by bool_tt x
@[simp] theorem or_true_right : (x || true) = true := by bool_tt x
@[simp] theorem or_not_self_left : (!x || x) = true := by bool_tt x
@[simp] theorem or_not_self_right : (x || !x) = true := by bool_tt x
@[simp] theorem or_idem : (x || x) = x := by bool_tt x
...
@[local simp] theorem or_deMorgan : (!(x || y)) = (!x && !y) := by bool_tt x y
...
@[simp] theorem xor_false_left : (false ^^ x) = x := by bool_tt x
@[simp] theorem xor_false_right : (x ^^ false) = x := by bool_tt x
@[simp] theorem xor_true_left : (true ^^ x) = !x := by bool_tt x
@[simp] theorem xor_true_right : (x ^^ true) = !x := by bool_tt x
@[simp] theorem xor_self : (x ^^ x) = false := by bool_tt x
@[simp] theorem xor_not_self_left : (!x ^^ x) = true := by bool_tt x
@[simp] theorem xor_not_self_right : (x ^^ !x) = true := by bool_tt x
...
theorem not_inj {x y : Bool} : (!x) = (!y) → x = y := by bool_tt using simp x y
