...
theorem foldl_zero (f : α → Fin 0 → α) (x) : foldl 0 f x = x := by
  simp [foldl, foldl.loop]
...
theorem foldl_eq_foldl_list (f : α → Fin n → α) (x) : foldl n f x = (list n).foldl f x := by
  ...
  | zero => rw [list_zero, foldl_zero, List.foldl_nil]
...
theorem foldr_loop (f : Fin (n+1) → α → α) (x) (h : m+1 ≤ n+1) :
  ...
  | succ m ih => rw [foldr_loop_succ, ih, foldr_loop_succ]; rfl

theorem foldr_zero (f : Fin 0 → α → α) (x) :
    foldr 0 f x = x :=
  by simp [foldr, foldr_loop_zero]
...
theorem foldr_eq_foldr_list (f : Fin n → α → α) (x) : foldr n f x = (list n).foldr f x := by
  induction n with
  | zero => rw [list_zero, foldr_zero, List.foldr_nil]
