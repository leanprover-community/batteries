...
private unsafe inductive ListMImpl (m : Type u → Type u) (α : Type u) : Type u
  ...
  | squash : m (ListMImpl m α) → ListMImpl m α
...
partial def fix [Monad m] (f : α → m α) (x : α) : ListM m α := cons x <| squash <| fix f <$> f x
...
/-- Repeatedly apply a function `f : α → m (α × List β)` to an initial `a : α`,
accumulating the elements of the resulting `List β` as a single monadic lazy list. -/
def fixl [Monad m] (f : α → m (α × List β)) (s : α) : ListM m β :=
  fixlWith f s []
...
def ofListM [Monad m] : List (m α) → ListM m α
  | [] => nil
  | h :: t => squash <| return cons (← h) (ofListM t)
...
-/
@[specialize]
def cases [Monad m] (xs : ListM m α) (hnil : ListM m β) (hcons : α → ListM m α → ListM m β) :
    ListM m β :=
  squash <| return match ← xs.uncons with
    | none => hnil
...
partial def take [Monad m] (xs : ListM m α) : Nat → ListM m α
  | 0 => empty
  | n+1 => xs.cases nil fun h l => cons h (l.take n)

/-- Drop the first `n` elements. -/
def drop [Monad m] (xs : ListM m α) : Nat → ListM m α
  | 0 => xs
  | n+1 => xs.cases nil fun _ l => l.drop n
...
partial def mapM [Monad m] (f : α → m β) (xs : ListM m α) : ListM m β :=
  xs.cases nil fun x xs => squash <| return cons (← f x) (xs.mapM f)
...
partial def filterMapM [Monad m] (f : α → m (Option β)) (xs : ListM m α) : ListM m β :=
  xs.cases nil fun x xs => squash do match ← f x with
...
partial def append [Monad m] (xs ys : ListM m α) : ListM m α :=
...
partial def join [Monad m] (xs : ListM m (ListM m α)) : ListM m α :=
  xs.cases nil fun x xs => append x (join xs)
...
partial def enumFrom [Monad m] (n : Nat) (xs : ListM m α) : ListM m (Nat × α) :=
  xs.cases nil fun x xs => cons (n, x) (xs.enumFrom (n+1))
...
partial def chunk [Monad m] (L : ListM m α) (n : Nat) : ListM m (Array α) :=
  go n #[] L
where
...
def concat [Monad m] (L : ListM m α) (a : α) : ListM m α := L.append (cons a nil)

/-- Take the product of two monadic lazy lists. -/
partial def zip [Monad m] (L : ListM m α) (M : ListM m β) : ListM m (α × β) :=
  L.cases nil fun a L =>
  M.cases nil fun b M =>
  cons (a, b) (L.zip M)
...
def head? [Monad m] (L : ListM m α) : m (Option α) := do
  pure <| (← L.uncons).map (·.1)
