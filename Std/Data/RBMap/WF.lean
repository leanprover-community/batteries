...
theorem insert_setBlack {t : RBNode α} :
  ...
  unfold insert cond; split <;> simp [setBlack_idem]
...
class IsCut (cmp : α → α → Ordering) (cut : α → Ordering) : Prop where
  ...
  lt_trans [TransCmp cmp] : cmpLt cmp x y → cut x = .lt → cut y = .lt
  /-- The set `{x | cut x = .gt}` is upward-closed. -/
  gt_trans [TransCmp cmp] : cmpLt cmp x y → cut y = .gt → cut x = .gt

/-- A "representable cut" is one generated by `cmp a` for some `a`. This is always a valid cut. -/
instance (cmp) (a : α) : IsCut cmp (cmp a) where
  lt_trans h₁ h₂ := TransCmp.lt_trans h₂ h₁.1
  gt_trans h₁ h₂ := TransCmp.gt_trans h₂ (OrientedCmp.cmp_eq_gt.2 h₁.1)
...
theorem zoom_fill' (cut : α → Ordering) (t : RBNode α) (path : Path α) :
  ...
  | nil => rfl
  | node c a y b iha ihb => unfold zoom; split; {apply iha}; {apply ihb}; {rfl}
...
theorem zoom_ins {t : RBNode α} {cmp : α → α → Ordering} :
    ...
    path.ins (t.ins cmp v) = ins path' (t'.setRoot v) := by
...
--     (h : t.Balanced c n) : ∃ c n, (t.alter cut f).Balanced c n :=
--   sorry
