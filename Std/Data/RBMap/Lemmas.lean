...
theorem IsCut.lt_trans [IsCut cmp cut] [TransCmp cmp]
  ...
  IsCut.le_lt_trans <| cmp_gt_asymm <| cmp_gt_lt_symm.2 H

theorem IsCut.gt_trans [IsCut cmp cut] [TransCmp cmp]
    (H : cmp x y = .lt) : cut y = .gt → cut x = .gt :=
  IsCut.le_gt_trans <| cmp_gt_asymm <| cmp_gt_lt_symm.2 H

theorem IsCut.congr [IsCut cmp cut] [TransCmp cmp] (H : cmp x y = .eq) : cut x = cut y := by
  cases ey : cut y
  · exact IsCut.le_lt_trans (fun h => nomatch H.symm.trans <| cmp_gt_lt_symm.1 h) ey
  · cases ex : cut x
    ...
    · refine IsCut.le_gt_trans (cmp := cmp) (fun h => ?_) ex |>.symm.trans ey
      cases H.symm.trans <| cmp_gt_lt_symm.1 h
...
instance (cmp) (a : α) : IsStrictCut cmp (cmp a) where
  le_lt_trans h₁ h₂ := cmp_lt_le_trans h₂ h₁
  le_gt_trans h₁ := Decidable.not_imp_not.1 (cmp_le_trans · h₁)
  exact h := (cmp_congr_left h).symm
...
theorem Ordered.unique [@TransCmp α cmp] (ht : Ordered cmp t)
  ...
  | node _ l _ r ihl ihr =>
    ...
    · rfl
    · cases e.symm.trans <| cmp_gt_lt_symm.2 (All_def.1 lx _ hy).1
    ...
    · cases e.symm.trans <| cmp_gt_lt_symm.2 (All_def.1 xr _ hx).1
    · cases e.symm.trans <| cmp_gt_lt_symm.2
...
theorem Ordered.lowerBound?_least_lb [@TransCmp α cmp] [IsCut cmp cut] (h : Ordered cmp t)
  ...
  | node _ _ _ _ ihl ihr =>
    ...
    · rcases lowerBound?_mem_lb h₁ with h₁ | ⟨⟨⟩⟩
      · cases cmp_lt_asymm h₂ (All_def.1 h.2.1 _ h₁).1
      · cases cmp_lt_asymm h₂ h₂
    · refine (cmp_lt_asymm h₂ ?_).elim; have := (All_def.1 h.1 _ hy').1
      rcases lowerBound?_mem_lb h₁ with h₁ | ⟨⟨⟩⟩
      · exact cmp_lt_trans this (All_def.1 h.2.1 _ h₁).1
      · exact this
    · exact ihr h.2.2.2 (by rintro _ ⟨⟨⟩⟩; exact h.2.1) h₁ hy' hx h₂
    · cases h₁; cases cmp_lt_asymm h₂ h₂
...
theorem Ordered.memP_iff_lowerBound? [@TransCmp α cmp] [IsCut cmp cut] (ht : Ordered cmp t) :
  ...
  · cases e : cmp x y
    ...
    · cases ey.symm.trans <| IsCut.gt_trans (cmp_gt_lt_symm.1 e) ex
...
theorem Ordered.lowerBound?_lt [@TransCmp α cmp] [IsStrictCut cmp cut] (ht : Ordered cmp t)
  ...
  refine ⟨fun h => ?_, fun h => cmp_gt_lt_symm.1 ?_⟩
  · cases e : cut x
