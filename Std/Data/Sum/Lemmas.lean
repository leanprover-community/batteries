...
theorem forall_sum_pi {γ : α ⊕ β → Sort _} (p : (∀ ab, γ ab) → Prop) :
...
@[simp] theorem not_isLeft (x : α ⊕ β) : not x.isLeft = x.isRight := by cases x <;> rfl
...
@[simp] theorem not_isRight (x : α ⊕ β) : !x.isRight = x.isLeft := by cases x <;> rfl
