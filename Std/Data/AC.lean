...
theorem Context.mergeIdem_head2 (h : x ≠ y) :
    mergeIdem (x :: y :: ys) = x :: mergeIdem (y :: ys) := by
  simp [mergeIdem, mergeIdem.loop, h]

theorem Context.evalList_mergeIdem (ctx : Context α) (h : ContextInformation.isIdem ctx)
    (e : List Nat) : evalList α ctx (mergeIdem e) = evalList α ctx e := by
...
theorem Context.evalList_sort_cons
  ...
  | nil => simp [sort.loop, evalList_insert ctx h]
  | cons z zs ih =>
    rw [evalList_sort_loop_swap ctx h]; simp [sort.loop, ←ih]; apply evalList_sort_congr ctx h
    rw [evalList_insert ctx h]
...
theorem Context.evalList_sort (ctx : Context α) (h : ContextInformation.isComm ctx) (e : List Nat) :
    evalList α ctx (sort e) = evalList α ctx e := by
  ...
  | step x y ys ih =>
    ...
    | nil =>
      simp [sort, sort.loop] at *; apply absurd h₂; apply sort_loop_nonEmpty; apply insert_nonEmpty
    | cons z zs => simp [evalList, ←h₂, ih]

theorem Context.toList_nonEmpty (e : AC.Expr) : e.toList ≠ [] := by
  induction e with
...
theorem Context.unwrap_isNeutral {ctx : Context α} {x : Nat} :
    ContextInformation.isNeutral ctx x = true →
      LawfulIdentity (EvalInformation.evalOp ctx) (EvalInformation.evalVar (β := α) ctx x) := by
  simp [ContextInformation.isNeutral, Option.isSome, EvalInformation.evalOp,
    EvalInformation.evalVar]
...
theorem Context.evalList_removeNeutrals (ctx : Context α) (e : List Nat) :
    evalList α ctx (removeNeutrals ctx e) = evalList α ctx e := by
  induction e using List.two_step_induction with
  | empty => rfl
  | single =>
...
theorem Context.evalList_append (ctx : Context α) (l r : List Nat) (h₁ : l ≠ []) (h₂ : r ≠ []) :
  ...
  | step x y ys ih =>
    simp [List.append, evalList, EvalInformation.evalOp] at *; rw [ih]; simp [ctx.assoc.1]

theorem Context.eval_toList (ctx : Context α) (e : AC.Expr) :
    evalList α ctx e.toList = eval α ctx e := by
  induction e with
  | var x => rfl
  | op l r ih₁ ih₂ =>
...
theorem Context.eval_norm (ctx : Context α) (e : AC.Expr) :
    evalList α ctx (norm ctx e) = eval α ctx e := by
  simp [norm]
  cases h₁ : ContextInformation.isIdem ctx <;> cases h₂ : ContextInformation.isComm ctx <;>
    simp_all [evalList_removeNeutrals, eval_toList, toList_nonEmpty, evalList_mergeIdem,
      evalList_sort]

theorem Context.eq_of_norm (ctx : Context α) (a b : AC.Expr) (h : norm ctx a == norm ctx b) :
    eval α ctx a = eval α ctx b := by
