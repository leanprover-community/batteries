...
theorem mem_cons_iff (a y : α) (l : List α) : a ∈ y :: l ↔ (a = y ∨ a ∈ l) := mem_cons
...
theorem eq_or_mem_of_mem_cons {a y : α} {l : List α} : a ∈ y :: l → a = y ∨ a ∈ l := by
  simp; exact id
...
theorem not_bex_nil (p : α → Prop) : ¬∃ x ∈ @nil α, p x := fun.

theorem ball_nil (p : α → Prop) : ∀ x ∈ @nil α, p x := fun.

theorem bex_cons (p : α → Prop) (a : α) (l : List α) :
  (∃ x ∈ a :: l, p x) ↔ p a ∨ ∃ x ∈ l, p x :=
  ⟨fun
    | ⟨_, Mem.head .., px⟩ => Or.inl px
    | ⟨x, Mem.tail _ h, px⟩ => Or.inr ⟨x, h, px⟩,
  fun o => o.elim
...
theorem ball_cons (p : α → Prop) (a : α) (l : List α) : (∀ x ∈ a :: l, p x) ↔ p a ∧ ∀ x ∈ l, p x :=
  ⟨fun al => ⟨al a (mem_cons_self _ _), fun x h => al x (mem_cons_of_mem _ h)⟩,
    fun ⟨pa, al⟩ x o => by cases o; exact pa; exact al x ‹_›⟩
