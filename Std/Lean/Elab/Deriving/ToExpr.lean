...
`Std.Lean.ToLevel` class. To use `ToExpr` in places where there is universe polymorphism, make sure
to have a `[ToLevel.{u}]` instance available.

**Warning:** Import `Std.Tactic.ToExpr` instead of this one. This ensures that you are using
...
def mkAuxFunction (ctx : Deriving.Context) (i : Nat) : TermElabM Command := do
  ...
  if ctx.usePartial then
    ...
    body ← Deriving.mkLet letDecls body
